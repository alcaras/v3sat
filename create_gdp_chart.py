#!/usr/bin/env python3
"""
Create a beautiful GDP chart from Victoria 3 time series data
"""
...
def create_gdp_chart(csv_file, output_file=None, log_scale=False):
    """Create a pretty GDP chart from CSV data"""
    ...
    df['date'] = df['date_index'].apply(lambda x: game_start + timedelta(days=x * 7))
    ...
    # Get GDP columns
    gdp_columns = [col for col in df.columns if col.endswith('_gdp')]
    countries = [col.replace('_gdp', '') for col in gdp_columns]
    
    # Plot each country's GDP over time
    for col, country in zip(gdp_columns, countries):
    ...
    if log_scale:
        ax.set_yscale('log')
        ax.set_ylabel('GDP (Millions, log scale)', fontsize=14, fontweight='bold')
        # For log scale, use simpler formatting
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'${x:g}M'))
    else:
        ax.set_ylabel('GDP (Millions)', fontsize=14, fontweight='bold')
    ...
    for col, country in zip(gdp_columns, countries):
        ...
        if len(country_data) >= 2:
            start_gdp = float(country_data.iloc[0])
            end_gdp = float(country_data.iloc[-1])
            growth = (end_gdp / start_gdp - 1) * 100
            print(f"{country:3s}: ${start_gdp/1_000_000:6.1f}M → ${end_gdp/1_000_000:6.1f}M ({growth:+5.1f}%)")

def main():
    parser = argparse.ArgumentParser(description="Create GDP chart from Victoria 3 data")
    ...
    if not args.csv_file:
        ...
        csv_files = list(reports_dir.glob("gdp_timeseries_*.csv"))
        if not csv_files:
    ...
    if not args.output:
        ...
        args.output = str(csv_path.parent / f"{csv_path.stem}_chart.png")
    
    # Create the chart
    create_gdp_chart(args.csv_file, args.output, args.log)
